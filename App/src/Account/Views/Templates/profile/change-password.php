...
<?php if ($change_password_view_model->shouldShowPasswordUpdateWarning()) { ?>
    <div class="card alert-warning">
        <div class="card-body">
            <?php if ($change_password_view_model->isFirstTimeUser()): ?>
                <h4 class="fw-bold text-dark">Welcome to <?= config('app.name') ?>!</h4>
                <p class="text-dark">It looks like this is your first time logging in or you are using a temporary password. For your security, please take a moment to set your permanent password below.</p>
            <?php else: ?>
                <h4 class="fw-bold text-dark">Time to Update Your Password!</h4>
                <p class="text-dark">Hey there! Just a friendly heads-up that it's been over <?= config('auth.password_max_age_days') ?> days since your last password update. For your security, our policy <strong>requires</strong> that you update it regularly.</p>
